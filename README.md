...
- `index.html` — the site
- `favicon.svg`, `apple-touch-icon.png` — from the brand kit
- `brand/` — the handful of logo exports the site serves (og:image, press).
  These are copies; the masters live in the studio's `brand/` folder and are
  regenerated by `build-brand.py`. Don't edit them here.
